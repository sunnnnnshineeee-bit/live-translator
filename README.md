...
| Translations arrive several sentences late | Watch the `Whisper took Xs for Ys of audio` line in the backend terminal. If X is larger than Y, the PC can't transcribe in real time — the app now merges queued chunks into single Whisper calls and drops what is still too old, and thread counts now match the CPU automatically, so latency stays bounded. Further options: close CPU-heavy apps, set `-np 1` in `scripts/start-llama`, or download the even smaller `ggml-tiny-q5_1.bin` from the same whisper.cpp Hugging Face repo into `whisper.cpp/models/` and start the backend with `$env:WHISPER_MODEL="whisper.cpp/models/ggml-tiny-q5_1.bin"` |
